...
//        String fieldMessage = fieldErrors.stream().map(FieldError::getDefaultMessage).findFirst().orElse(exception.getMessage());
        ...
        return new ResponseEntity<>(
                HttpResponse.builder()
                        .timestamp(now().toString())
                        .reason(fieldMessage)
                        //long message, not recommended in development
//                        .developerMessage(exception.getClass().getName())
